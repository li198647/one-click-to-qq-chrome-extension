...
def tray_win():
    return find(lambda w: w["cls"].startswith("QQBridgeTrayWnd_"))
...
def main():
    ...
    if st["tray_found"]:
        ...
        for _ in range(24):
            time.sleep(0.25)
            con = console_win()
            if con and con["vis"] and has_sysmenu(con["hwnd"]) and not tray_win():
                back = con
                break
        check("2: 托盘消息窗口已消失", tray_win() is None)
